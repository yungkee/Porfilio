...
export function Wizard(props: JSX.IntrinsicElements["group"]) {
  ...
  const modelRef = useRef<THREE.Group>(null); // 创建对3D模型的引用

  // 添加浮动动画效果
  useFrame((state) => {
    if (!modelRef.current) return;
    // 每帧更新模型的Y轴位置,创造上下浮动的效果
    modelRef.current.position.y =
      -1.5 + Math.sin(state.clock.elapsedTime) * 0.15; // 使用正弦函数创造周期性的浮动
  });
  ...
  return (
    <group
      ...
      ref={modelRef}
      dispose={null}
      position={[0, -1.5, 0]} // 设置模型的初始位置
      rotation={[0.25, 0, 0]} // 设置模型的旋转角度>
      scale={[0.06, 0.06, 0.06]} // 设置模型的缩放比例
    >
      <mesh
        castShadow
        receiveShadow
